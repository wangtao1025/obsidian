...
├── frontend/   # 前端相关（Vue、React、Next.js、JS、TS、Vite 等，当前占位）
├── ai/         # AI 相关
├── linux/      # Linux 相关
├── docker/     # Docker 相关
└── …           # 后续可新增分类，如 database/ 等
```

- **顶栏**：按分类切换（Python / 前端 / AI / Linux / Docker），不列单篇文档，避免与侧栏重复。
...
- **站点根 `/`**：由 `rewrites` 映射为「学习路径总览」页（`学习路径总览.md`）；语法手册见 [语法手册总览](/python/python语法手册)。
- **Python 分类**：顶栏「Python」指向 `/python/`；侧栏分组为「入口」「语法手册（按章）」「标准库」「第三方库（课程内）」「自测与面试」。
- **前端 分类**：顶栏「前端」指向 `/frontend/`；侧栏为「首页」「占位目录」；当前为占位（Vue、React、Next.js、JS、TS、Vite 等待补充）。
