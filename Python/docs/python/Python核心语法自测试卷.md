...
**6.11** Python 有哪六种关系运算符？它们返回什么类型的值？
> 你的回答：

**6.12** 链式比较 `1 < x < 10` 等价于什么表达式？中间变量 `x` 会求值几次？
> 你的回答：

**6.13** 为什么不能直接用 `==` 比较两个浮点数是否相等？正确的做法是什么？
> 你的回答：

**6.14** 集合（set）和字典（dict）支持哪些关系运算符？使用 `<` 或 `>` 会怎样？
> 你的回答：

**6.15** 数值比较：整数与浮点数能否直接比较？`10 == 10.0` 的结果是什么？为什么？
> 你的回答：

**6.16** 字符串比较：Python 中字符串按什么规则比较大小？`'Apple' < 'apple'` 的结果是 True 还是 False？原因是什么？
> 你的回答：

**6.17** 列表比较：`[1, 2, 3] < [1, 2, 4]` 的结果是什么？`[1, 2]` 和 `[1, 2, 3]` 谁“小”？简述列表比较的规则。
> 你的回答：

**6.18** 如何用列表比较实现版本号大小比较（如 `"3.9.1"` 与 `"3.10.0"`）？请写出关键代码或思路。
> 你的回答：

**6.19** 链式比较：`a < b > c` 等价于什么布尔表达式？是否会比较 `a` 与 `c`？
> 你的回答：

**6.20** 链式比较是否会短路？若 `a < b` 已经为 False，`b < c` 还会被求值吗？
> 你的回答：

**6.21** 在 Python 中，哪些类型支持全部六种关系运算符（`<` `<=` `>` `>=` `==` `!=`）？哪些类型只支持 `==` 和 `!=`？
> 你的回答：

**6.22** `{1, 2} == {2, 1}` 的结果是什么？为什么？`{1} < {2}` 会怎样？
> 你的回答：

**6.23** 为什么 `0.1 * 3 == 0.3` 可能得到 False？判断两个浮点数“相等”时推荐用什么方式？
> 你的回答：

**6.24** 写出判断两个浮点数近似相等的两种写法：① 使用 `math.isclose`；② 使用 `abs(a - b)` 与容差。
> 你的回答：

**6.25** 嵌套列表：`[[1,2],[3,4]] < [[1,2],[3,5]]` 的结果是什么？嵌套字典（如 `{"a": 1, "b": {"x": 2}}`）支持用 `<` 比较吗？
> 你的回答：

**6.26** 自定义类要支持 `p1 < p2` 比较，需要实现哪个魔术方法？若只实现 `__lt__`，`sorted()` 能否对该类实例列表排序？
> 你的回答：

**6.27** 自定义类在比较时，若对方类型不支持（如与 int 比较），应返回什么，以便 Python 尝试“反向”比较？
> 你的回答：

**6.28** 用关系运算符与逻辑运算符写出：① 判断“年龄在 18 到 65 之间（含边界）”的 if 条件；② 用列表推导筛选出在闭区间 `[min_val, max_val]` 内的元素。
> 你的回答：
...
**4.14** 会回退调用 `__repr__`。如果 `__str__` 未定义，Python 会自动使用 `__repr__` 的返回值。（待复习）
...
**5.2** `key` 参数，如 `max(students, key=lambda x: x["score"])`（待复习）
...
**6.4** `or` 返回第一个真值，全假则返回最后一个。输出：`hello`、`1`、`[]`（待复习）

**6.5** `None`、`False`、`0`、`0.0`、`0j`、`''`、`[]`、`()`、`{}`、`set()`、`range(0)`（待复习）
...
**6.8** 1. `__bool__()` 2. `__len__()` 3. 默认为 `True`（待复习）

**6.9** `not` 必定返回布尔值（`True`/`False`）；`and`/`or` 返回的是操作数本身，不一定是布尔值。

**6.10** `True`、`False`、`True`、`True`

**6.11** 六种：`<`、`<=`、`>`、`>=`、`==`、`!=`。都返回布尔值（True 或 False）。

**6.12** 等价于 `(1 < x) and (x < 10)`。中间变量 `x` 只求值一次。

**6.13** 浮点数用二进制近似存储，存在精度误差，直接用 `==` 可能得到 False。应用 `math.isclose(a, b)` 或 `abs(a - b) < 1e-9` 判断是否“足够接近”。

**6.14** 集合和字典只支持 `==` 和 `!=`（判断是否相等）。使用 `<`、`>`、`<=`、`>=` 会抛出 TypeError（集合、字典无顺序概念）。

**6.15** 可以。`10 == 10.0` 为 True。整数与浮点数比较时会发生自动类型提升，按数值是否相等比较。

**6.16** 按字典序（Unicode 码点）逐字符比较，区分大小写。`'Apple' < 'apple'` 为 True，因为 `'A'` 的码点小于 `'a'`。

**6.17** `[1, 2, 3] < [1, 2, 4]` 为 True。`[1, 2]` 小于 `[1, 2, 3]`（前面相同则较短者小）。规则：从左到右逐元素比较，首对不相等即得结果；若前缀相同则较短者小。

**6.18** 将版本号按 `.` 拆成整数列表再比较。例：`v1 = [int(x) for x in "3.9.1".split('.')]`，`v2 = [int(x) for x in "3.10.0".split('.')]`，用 `v1 < v2` 即可。

**6.19** 等价于 `(a < b) and (b > c)`。不会比较 a 与 c，只对相邻对象两两比较。

**6.20** 会短路。从左到右求值，一旦某一段为 False 即停止，后面的比较不再求值。

**6.21** 支持全部六种：int、float、str、list、tuple 等（同类型或 int 与 float）。只支持 `==`、`!=`：set、dict（无顺序概念）。

**6.22** `{1, 2} == {2, 1}` 为 True，集合无序，只要元素相同即相等。`{1} < {2}` 会抛出 TypeError，集合不支持 `<`。

**6.23** 浮点数用二进制近似存储，存在精度误差，`0.1*3` 与 `0.3` 在内部表示可能略有差异。推荐用 `math.isclose(a, b)` 或容差判断 `abs(a - b) < 1e-9`。

**6.24** ① `math.isclose(a, b)`；② `abs(a - b) < 1e-9`（或其它容差如 `1e-6`）。

**6.25** `[[1,2],[3,4]] < [[1,2],[3,5]]` 为 True（按元素逐层比较）。嵌套字典不支持 `<`，只支持 `==`、`!=`，键值对完全一致才判等。

**6.26** 需实现 `__lt__`（小于）。只实现 `__lt__` 时，`sorted()` 可以对该类实例排序，Python 会使用 `__lt__` 进行排序。

**6.27** 应返回 `NotImplemented`，这样 Python 会尝试调用对方类型的比较方法（如 `other.__gt__(self)`），实现双向比较。

**6.28** ① `if 18 <= age <= 65:` 或 `if not (18 <= age <= 65): errors.append("...")`；② `[x for x in data_list if min_val <= x <= max_val]`。
...
**8.3** 核心算法：`while` 循环，数字 ≥ 1024 就除以 1024.0 并将单位索引 +1。单位阶梯：`['B', 'KiB', 'MiB', 'GiB', 'TiB']`（待复习）
