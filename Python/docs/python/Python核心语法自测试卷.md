...
**2.1** 有序、可变、支持重复元素、支持索引与切片、元素可为任意类型。

**2.2** `append(x)` 将 `x` 作为单个元素添加到末尾；`extend(iterable)` 将可迭代对象中的每个元素逐一追加到末尾。

**2.3** `pop(i)` 按索引删除并返回该元素（默认最后一个）；`remove(x)` 按值删除第一个匹配项，不返回值。

**2.4** `.sort()` 原地排序，修改原列表，返回 `None`；`sorted()` 返回新列表，不修改原数据。
...
**2.16** `.find()` 找不到返回 `-1`；`.index()` 找不到抛出 `ValueError`。
...
**2.19** `.split()` 不传参数会合并所有连续空白字符；`.split(" ")` 严格按单个空格分割，连续空格会产生空字符串元素。
...
**2.30** 计算机底层使用二进制存储小数，很多十进制小数无法精确表示为有限位二进制小数。
...
**3.5** `UnicodeEncodeError`：字符串编码为字节时失败（如用 ASCII 编码中文）。`UnicodeDecodeError`：字节解码为字符串时失败（如用 UTF-8 解码 GBK 字节流）。

**3.6** 当 `try` 块没有发生任何异常时执行。
...
**4.4** 不指定对齐方式会产生语法歧义。`f"{n:1>10d}"` 表示用 `1` 填充、右对齐、宽度 10；`f"{n:110d}"` 会被解析为宽度 110、默认空格填充。
...
**9.2** ASCII 字符 1 字节，汉字 3 字节，Emoji 4 字节。
...
**9.4** 遇到无法识别的字符时用替代字符代替，防止整个程序崩溃。

**9.5** `ord('中')` → `20013`；`chr(65)` → `'A'`
