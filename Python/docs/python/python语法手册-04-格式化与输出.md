...
**格式说明符**：`{变量:填充字符对齐方式宽度.精度类型}`（**自测卷 5.1 待复习**）
- `:` - 格式说明符开始
- `.2` - 精度（保留2位小数）
- `f` - 格式类型（浮点数）
- `<` - 左对齐，`>` - 右对齐，`^` - 居中
...
#### 1.12  `__str__` 和 `__repr__`（自测卷 5.13、5.14、5.15 待复习）
