...
**本章对应自测卷**：[三、内置数据结构（共 31 题）](/python/Python核心语法自测试卷#三内置数据结构-共-31-题)  
**学完能做什么**：熟练用列表/元组/字符串/字典/集合做增删改查，分清 `.sort()` 与 `sorted()`、`.pop()` 与 `.remove()` 等。  
**小白注意**：① `.sort()` 改原列表且返回 `None`，别写 `a = lst.sort()`；要新列表用 `sorted()`。② 字典的 `in` 默认只查键不查值。③ 元组单元素要写 `(x,)` 否则是括号表达式。
...
#### 1.5.1 **查找替换**：
  - `.find("sub")` / `.index("sub")`
	  - 查找索引（find 失败返 -1，index 失败报错）。（**自测卷 3.16 待复习**）
  - `.replace("old", "new")`
	  - 替换子串。
#### 1.5.2 **检查与转换**：
