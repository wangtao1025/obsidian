...
- **Python**：语法手册（含第13章标准库）、标准库与第三方库（课程内单篇）、自测与面试均在此分类。  
...
1. **Python 语法**：变量、函数、类、异常、字典/列表即可；可跳读 [语法手册总览](/python/python语法手册)，第 13 章标准库扫一眼。
...
1. **数据库**：先会用一种关系型 DB（如 SQLite 或 PostgreSQL）做增删改查与简单表设计；再考虑 ORM（如 SQLAlchemy）减少手写 SQL。本站以 AI 课程为主，DB 可结合 [语法手册第 13 章](/python/python语法手册-11-标准库) 与外部教程。
...
| 标准库 | 第13章总览 + math、typing、collections、random、abc、dataclasses、heapq | [Python 首页](/python/)「标准库」表 |
